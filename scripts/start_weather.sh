...
if ! ps auxw | grep "/usr/local/bin/weather.py" | grep -v -e grep -e SCREEN -e inside_weather; then  
    ...
    screen -S weather -d -m /usr/local/bin/weather.py --log --period=3 --logfile=$HOME/data/temperature/broadcast/weather.txt
else
...
if ! ps auxw | grep "/usr/local/bin/weather.py" | grep inside_weather | grep -v -e grep -e SCREEN; then  
    ...
    cd $HOME/data/temperature/broadcast
    screen -S weather -d -m /usr/local/bin/weather.py --log --period=3 --logfile=$HOME/data/temperature/broadcast/inside_weather.txt --server=192.168.88.13
else
