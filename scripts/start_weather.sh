...
if ! ps auxw | grep "/usr/local/bin/weather.py" | grep -v -e grep -e SCREEN -e inside_weather; then  
    ...
    screen -S weather -d -m /usr/local/bin/weather.py --server-type='outside' --log --period=60 --logfile=$HOME/data/temperature/broadcast/weather.txt --server=192.168.88.18
else
