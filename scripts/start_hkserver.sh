...
#
if ! ps auxw | grep "/usr/bin/python /usr/local/bin/run_hkserver.py" | grep -v -e grep -e SCREEN; then  
    ...
    screen -S hkserver -d -m /usr/bin/python /usr/local/bin/run_hkserver.py
else
